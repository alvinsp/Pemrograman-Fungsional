...
def jumlah_film_berdasarkan_genre(movies):
    def filter_genre(movie):
        return movie["genre"] == genre

    genre_counts = {}
    for genre in set(map(lambda movie: movie["genre"], movies)):
        filtered_movies = filter(filter_genre, movies)
        count = len(list(filtered_movies))
        genre_counts[genre] = count
    return genre_counts

# use list reduce 
def rata_rata_rating_film_berdasarkan_tahun(movies):
    rating_tuples = list(map(lambda movie: (movie["year"], movie["rating"]), movies))
    rating_by_year = reduce(
        lambda acc, rating: {**acc, rating[0]: acc.get(rating[0], []) + [rating[1]]},
        rating_tuples,
        {},
    )
    avg_rating_by_year = {year: sum(ratings) / len(ratings) for year, ratings in rating_by_year.items()}
...
while True:
    ...
    if choice == '1':
        ...
        for genre, count in genre_counts.items():
            print(f"Genre: {genre}, Jumlah Film: {count}")
    elif choice == '2':
